...
public class AudioSegmentationService : IAudioSegmentationService
{
    ...
    )
    {
        ...
        try
        {
            ...
            var segmentBoundaries = await Task.Run(() => FindSegmentBoundariesParallel(samples));
    ...
    private async Task<List<float>> ProcessAudioStreamAsync(WaveStream waveProvider)
    {
        ...
        while (
            (samplesRead = await Task.Run(() => monoProvider.Read(buffer, 0, buffer.Length))) > 0
        )
        {
    ...
    private List<(TimeSpan Start, TimeSpan End)> FindSegmentBoundariesParallel(List<float> samples)
    {
    ...
    )
    {
        ...
        await Parallel.ForEachAsync(
            ...
            async (boundary, ct) =>
            {
                ...
                var segmentSamples = samples.Skip(startIndex).Take(endIndex - startIndex).ToList();
